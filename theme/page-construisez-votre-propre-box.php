	...
	<main id="main">
		...
		?>
			<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
				...
				<!-- Section produits -->
				<section class="shop-products px-5 py- bg-[#f5f1ed] md:px-10 pb-10">
					...
					<div class="featured-carousel mb-16">
						<div class="text-center mb-12">
							<h2 class="shop-title text-4xl md:text-5xl lg:text-6xl font-black uppercase pb-6 pt-10">Construisez votre box</h2>
						...
						if (class_exists('WooCommerce')) {
							...
							if ($region_categories && !is_wp_error($region_categories) && $total_carousel > 0) {
								...
								echo '<h3 class="text-2xl md:text-3xl lg:text-4xl font-black text-[#2d5a3d] uppercase">Choisissez votre box</h3>';
								echo '<div class="carousel-counter text-lg md:text-xl font-bold"><span id="carouselCurrent">1</span>/<span id="carouselTotal">' . esc_html($total_carousel) . '</span></div>';
								echo '</div>';
						...
						?>
									<div class="carousel-item w-full flex-shrink-0" data-region="<?php echo esc_attr($region_slug); ?>">
										<div class="flex flex-col lg:flex-row items-center justify-between gap-6 md:gap-8 py-8 md:py-12 px-4 md:px-8">
											<!-- Left: Region Info -->
											<div class="w-full lg:w-1/4 flex flex-col justify-center text-center lg:text-left">
												<p class="text-xs md:text-sm italic text-gray-600 mb-2">Soleil et convivialité</p>
												<h4 class="text-3xl md:text-4xl lg:text-5xl font-black mb-6 leading-tight text-[#2d5a3d]"><?php echo esc_html($region_name); ?></h4>
												<div class="flex items-center justify-between gap-3 md:gap-4">
													<button type="button" class="select-region px-5 md:px-8 py-2 md:py-3 bg-[#2d5a3d] text-white rounded-full font-bold uppercase text-xs md:text-sm w-fit" data-region="<?php echo esc_attr($region_slug); ?>">
														Choisir cette box
													</button>
													<div class="flex items-center gap-2">
														<button class="carousel-prev flex-shrink-0 p-1 md:p-2 hover:opacity-70 transition-opacity">
															<svg width="20" height="18" viewBox="0 0 29 25" fill="none" xmlns="http://www.w3.org/2000/svg" class="scale-x-[-1] w-full h-full">
														...
														<button class="carousel-next flex-shrink-0 p-1 md:p-2 hover:opacity-70 transition-opacity">
															<svg width="20" height="18" viewBox="0 0 29 25" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-full h-full">
																<path d="M20.3653 14.8754L19.3432 15.3075C16.4458 16.7624 14.2736 20.33 14.0782 23.5478C14.074 23.6196 14.0523 23.7473 14.1305 23.7763L19.4578 25C19.5106 22.6307 20.1894 20.2023 21.7113 18.3602C23.4931 16.2035 26.251 15.1213 29 14.8754L29 10.112C26.2494 9.83388 23.4894 8.75055 21.7118 6.57756C20.2079 4.73911 19.5186 2.35927 19.4589 8.50574e-07L14.0766 1.23953C14.3386 4.77552 16.4125 8.31363 19.6876 9.81806C19.8973 9.91462 20.1419 10.0233 20.3659 10.0629C20.3801 10.1843 20.3152 10.094 20.2877 10.094L0.0475394 10.094L0.000527078 10.141L0.000526871 14.8754L20.3653 14.8754Z" fill="#154F41" />
															</svg>
														</button>
													</div>
											...
											<div class="w-full lg:w-3/4 flex justify-center items-center">
												<div class="relative w-48 h-48 md:w-72 md:h-72 lg:w-96 lg:h-96 flex items-center justify-center">
