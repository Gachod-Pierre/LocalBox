<?php
/**
 * Simplified WooCommerce filter bar (without category selector).
 * Used for filtered product pages like "Construisez votre propre box"
 */
...
// Get current URL without category param
$current_url = remove_query_arg('product_cat');
?>
<form method="get" action="<?php echo esc_url($current_url); ?>" class="shop-filters flex gap-5 mb-12 flex-wrap items-center">
    <?php if ($attr_type) :
        ...
        $type_param = 'filter_' . $attr_type->attribute_name;
    ?>
    ...
    <div class="filter-actions flex gap-3 w-full md:w-auto">
        ...
        <a href="<?php echo esc_url($current_url); ?>" class="px-5 py-3 bg-white border-2 border-black rounded-full text-xs font-bold uppercase tracking-widest">Réinitialiser</a>
    </div>
</form>
...
(function(){
    ...
    form.addEventListener('submit', function(){
        // Remove empty params so URLs don't contain bare keys
        var candidates = Array.prototype.slice.call(form.querySelectorAll('[name^="filter_"],[name^="query_type_"],[name="orderby"]'));
